    ...
    <div class="p-6 max-w-5xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-sm mt-0.5" style="color: var(--text-muted)">{{ users().length }} members</p>
        </div>
        <button
      ...
      @if (showForm()) {
        <div class="border rounded-xl p-5 mb-6"
          ...
          <h2 class="text-sm font-medium mb-4" style="color: var(--text-primary)">New team member</h2>
          <form [formGroup]="form" (ngSubmit)="create()" class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
              <label class="text-xs uppercase tracking-wider" style="color: var(--text-muted)">Role</label>
            ...
            <div class="space-y-1">
              ...
              <div class="flex gap-2">
                <input formControlName="password" type="text" placeholder="Min. 8 characters"
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b" style="border-color: var(--border)">
                <th class="text-left text-xs uppercase tracking-wider px-4 py-3 font-medium" style="color: var(--text-muted)">Member</th>
                <th class="text-left text-xs uppercase tracking-wider px-4 py-3 font-medium" style="color: var(--text-muted)">Role</th>
                <th class="text-left text-xs uppercase tracking-wider px-4 py-3 font-medium hidden md:table-cell" style="color: var(--text-muted)">Last login</th>
        ...
        <div class="fixed inset-0 flex items-center justify-center z-50 px-4"
          ...
          <div class="border rounded-xl p-6 max-w-sm w-full"
            ...
            <p class="text-xs mb-4" style="color: var(--text-muted)">
              ...
              <span class="font-medium" style="color: var(--text-primary)">{{ resettingUser()!.name }}</span>.
      ...
      @if (deletingUser()) {
        <div class="fixed inset-0 flex items-center justify-center z-50 px-4"
          ...
          <div class="border rounded-xl p-6 max-w-sm w-full"
               style="background-color: var(--bg-surface); border-color: var(--border)">
            <h3 class="text-sm font-medium mb-2" style="color: var(--text-primary)">Remove member</h3>
            <p class="text-sm mb-5" style="color: var(--text-secondary)">
              Remove <span class="font-medium" style="color: var(--text-primary)">{{ deletingUser()!.name }}</span>
              from this team? They will lose access immediately.
            </p>
            <div class="flex justify-end gap-2">
              <button (click)="deletingUser.set(null)"
                class="text-sm px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
                style="color: var(--text-secondary)">
                Cancel
              </button>
...
export class UserList implements OnInit {
  ...
  create() {
    ...
    this.usersService.create(tenantId, this.form.getRawValue()).subscribe({
      ...
      error: (err) => {
        this.formError.set(
          err.status === 409 ? 'Email already taken.' : 'Something went wrong.',
        );
        this.saving.set(false);
      },
    });
