  ...
  template: `
    <div>
      <div class="mb-6">
        ...
        <p class="text-sm mt-0.5" style="color: var(--text-muted)">One key per environment. Used to authenticate the SDK.</p>
      </div>
        ...
        <div class="space-y-4">
          @for (item of envItems(); track item.env.id) {
            <div class="border rounded-xl overflow-hidden" style="border-color: var(--border)">
              ...
              <!-- Header del ambiente -->
              <div class="flex items-center gap-3 px-4 py-3" style="background-color: var(--bg-surface)">
                <div class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                     [style.background-color]="item.env.color ?? '#52525b'"></div>
                <span class="text-sm font-medium flex-1" style="color: var(--text-primary)">{{ item.env.name }}</span>
                <span class="text-xs" style="color: var(--text-muted)">{{ item.keys.length }} key{{ item.keys.length !== 1 ? 's' : '' }}</span>
                <button
              ...
              @if (item.keys.length > 0) {
                <div class="divide-y" style="border-color: var(--border-subtle)">
                  @for (key of item.keys; track key.id) {
                    <div class="flex items-center gap-4 px-4 py-3" style="border-color: var(--border-subtle)">
                      <div class="flex-1 min-w-0">
                        <p class="text-sm" style="color: var(--text-primary)">{{ key.name }}</p>
                        <p class="text-xs font-mono mt-0.5" style="color: var(--text-muted)">{{ key.keyPrefix }}_••••••••</p>
                      </div>
                      ...
                      @if (!key.isActive) {
                        <span class="text-xs px-2 py-0.5 rounded" style="color: var(--warning-fg); background-color: var(--warning-subtle)">Revoked</span>
                      }

                      @if (key.isActive) {
                        <button (click)="revoke(item.env, key)"
                          class="text-xs transition-colors cursor-pointer"
                          style="color: var(--text-muted)">
                          Revoke
                        </button>
                      }
                      <button (click)="confirmDelete(item.env, key)"
                        class="text-xs transition-colors cursor-pointer"
                        style="color: var(--text-muted)">
                        Delete
                      </button>
        ...
        <div class="fixed inset-0 flex items-center justify-center z-50 px-4" style="background-color: var(--bg-overlay)">
          <div class="border rounded-xl p-6 max-w-sm w-full" style="background-color: var(--bg-surface); border-color: var(--border)">
            <h3 class="text-sm font-medium mb-4" style="color: var(--text-primary)">
              Generate key for <span style="color: var(--accent-text)">{{ creatingForEnv()!.name }}</span>
            </h3>

            <form [formGroup]="form" (ngSubmit)="createKey()" class="space-y-4">
              <div class="space-y-1">
                <label class="text-xs uppercase tracking-wider" style="color: var(--text-muted)">Label</label>
                <input formControlName="name" placeholder="Android prod key"
      ...
      @if (newKeyValue()) {
        <div class="fixed inset-0 flex items-center justify-center z-50 px-4" style="background-color: var(--bg-overlay)">
          <div class="border rounded-xl p-6 max-w-md w-full" style="background-color: var(--bg-surface); border-color: var(--border)">
            <h3 class="text-sm font-medium mb-1" style="color: var(--text-primary)">API key generated</h3>
            <p class="text-xs mb-4" style="color: var(--text-muted)">
              Copy this key now. You won't be able to see it again.
            </p>
            <div class="rounded-lg px-3 py-2.5 font-mono text-xs break-all mb-4"
                 style="background-color: var(--mono-bg); color: var(--success-fg)">
              {{ newKeyValue() }}
            </div>
            <div class="flex justify-end">
              <button (click)="newKeyValue.set(null)"
      ...
      @if (deletingKey()) {
        <div class="fixed inset-0 flex items-center justify-center z-50 px-4" style="background-color: var(--bg-overlay)">
          <div class="border rounded-xl p-6 max-w-sm w-full" style="background-color: var(--bg-surface); border-color: var(--border)">
            <h3 class="text-sm font-medium mb-2" style="color: var(--text-primary)">Delete SDK key</h3>
            <p class="text-sm mb-5" style="color: var(--text-secondary)">
              This will permanently delete
              <span class="font-medium" style="color: var(--text-primary)">{{ deletingKey()!.key.name }}</span>.
              Any SDK using it will stop working immediately.
            </p>
            <div class="flex justify-end gap-2">
              <button (click)="deletingKey.set(null)"
                class="text-sm px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
                style="color: var(--text-secondary)">
                Cancel
              </button>
      ...
      this.sdkKeysService.findAll(this.projectId(), envId).subscribe({
        next: (keys) => {
          this._envItems.update((items) =>
            items.map((item) =>
              item.env.id === envId ? { ...item, keys } : item,
            ),
          );
